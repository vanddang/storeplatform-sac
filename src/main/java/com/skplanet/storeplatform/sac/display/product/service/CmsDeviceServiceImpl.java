...
public class CmsDeviceServiceImpl implements CmsDeviceService {
	...
	public void remappingDeviceProcess(DeviceMappingQueueVO message) throws StorePlatformException{
		...
		log.debug("*********************************************************");
		log.debug("CmsDeviceController process started!!");
		log.debug("***** getTxId         --> txId           :::: {}", message.getTxId());
		log.debug("***** getCommandCode  --> type           :::: {}", message.getCommandCode());
		log.debug("***** getModule       --> deviceModelCd  :::: {}", message.getModule());
		log.debug("*********************************************************");
		...
		cv.setResultMsg(messageSourceAccessor.getMessage("if.cms.msg.code." + cv.getResultCd()));
		
		log.info("Remapping Info = " + txId + " / " + type + " / " + deviceModelCd + " / " + contentList.size());
		...
		for (DeviceMappingContentVO dmcv : contentList) {
			...
			try{
				...
				log.info("Remapping Prod Info = " + prodId + " / " + subContentsId + " / " + deviceModelCd + " / " + lcdSize + " / " + vmVer);
				
				// 상품 정보 체크
				ProductVo dpProd = new ProductVo();
				dpProd.setProdId(prodId);
				int dpCount = commonDAO.queryForInt("Display_Device.selectDpProdCount", dpProd);
				if (dpCount <= 0) throw new StorePlatformException(IFConstants.CMS_RST_CODE_DP_DATA_INVALID_ERROR, "PROD_ID [ " + prodId + " ] 로 등록되어진 상품 정보가 없습니다.");
		...
		log.debug(":::::countSuccess {} , countFail {} :::::::", countSuccess, countFail);
		...
		log.debug("*********************************************************");
		log.debug("***** dmr.getResult         :::: {}", dmr.getResult());
		log.debug("***** dmr.getTxId           :::: {}", dmr.getTxId());
		log.debug("***** dmr.getMsg            :::: {}", dmr.getMsg());
		log.debug("CmsDeviceController process end!!");
		log.debug("*********************************************************");	
