...
public class DownloadAppServiceImpl implements DownloadAppService {
	 ...
	 */
	@Override
	public DownloadAppRes searchDownloadApp(SacRequestHeader requestheader, DownloadAppReq downloadAppReq) {
		...
		this.log.debug("###############################################################");
		this.log.debug("os version	:	" + temp[1]);
		this.log.debug("###############################################################");
		downloadAppReq.setTenantId(tanantHeader.getTenantId());
		downloadAppReq.setDeviceModelCd(deviceHeader.getModel());
		downloadAppReq.setOsVersion(osVersion);

		DownloadAppRes response = new DownloadAppRes();
		...
		if (downloadAppReq.getDummy() == null) {

			// dummy 호출이 아닐때
			DownloadApp downloadAppInfo = null;

			if ("package".equals(downloadAppReq.getFilteredBy())) {
				String prodId = (String) this.commonDAO.queryForObject("Download.getProductIdForPackageName",
						downloadAppReq);
				this.log.debug("###############################");
			...
			// 다운로드 앱 상품 조회
			downloadAppInfo = this.commonDAO.queryForObject("Download.getDownloadAppInfo", downloadAppReq,
					DownloadApp.class);
			...
			if (downloadAppInfo.getSeedProductId() == null || "".equals(downloadAppInfo.getSeedProductId())) {
				identifier.setText("");
			} else {
			...
			if (downloadAppInfo.getSeedCaseRefCd() == null || "".equals(downloadAppInfo.getSeedCaseRefCd())) {
