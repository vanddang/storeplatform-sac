...
public class CmsServiceImpl implements CmsService {
	...
	public void process(NotificationRefactoringSac message) throws StorePlatformException {
		...
		log.info("CMS MQ APP Process Start");
		
		CmsVo cv = new CmsVo();
		int transactionKey = message.getTransactionKey();
		log.info("CMS TransactionKey = " + transactionKey);
		...
		if (null != dpProd) {
			...
			if (!"".equals(prodId) && !"".equals(mbrNo)) {
				...
				try {
					 ...
					 */
					initializer.clear(message);
					
					/*
					 * 데이터 재구성
					 */
					builder.build(message, tempList);
				...
				log.info("CMS Result Code = " + cv.getResultCd());
				log.info("CMS Result Message = " + cv.getResultMsg());
			} else {
		...
		log.info("CMS Transaction Key = " + ntr.getTransactionKey());
		log.info("CMS Return Code = " + ntr.getDeployResultCd());
		log.info("CMS Return Message = " + ntr.getDeployResultDtlMsg());
