...
public class VodServceImpl implements VodService {
	...
	public VodDetailRes searchVod(VodDetailReq req) {
		...
		if (vodDetail != null) {
			...
			List<ProductImage> screenshotList = this.commonDAO.queryForList("VodDetail.selectSourceList", productImage,
					ProductImage.class);
			...
			// 2. subProjectList
			// --------------------------------------------------------
			List<VodDetail> subProductList = this.commonDAO.queryForList("VodDetail.selectVodSeries", req,
					VodDetail.class);
	...
	private void mapSubProductList(Product product, List<VodDetail> vodDetailList) {
		...
		/*
		 * Product product1 = new Product(); product1.setIdentifier(new
		 * Identifier(DisplayConstants.DP_CONTENT_IDENTIFIER_CD, "0002119673"));
		 * 
		 * Support support2 = new Support(); support2.setType("drm"); support2.setText("Y"); Support support3 = new
		 * Support(); support3.setType("hd"); support3.setText("Y"); product1.setSupportList(new
		 * ArrayList<Support>(Arrays.asList(support2, support3)));
		 * 
		 * product1.setTitle(new Title("무한도전"));
		 * 
		 * Date date2 = new Date(); date2.setType("date/reg"); date2.setText("20130324T220917+0900"); Date date3 = new
		 * Date(); date3.setType("date/saleReg"); date3.setText("20130324T220917+0900"); product1.setDateList(new
		 * ArrayList<Date>(Arrays.asList(date2, date3)));
		 * 
		 * 
		 * Support emplySupport = new Support(); Rights rights2 = new Rights(); rights2.setPlay(new Play(emplySupport,
		 * new Price(700), new Date("date/publish", "2013"))); rights2.setStore(new Store(emplySupport, new Price(700),
		 * new Date("date/publish", "2013"))); rights2.setDate(new Date("date/publish", "2013"));
		 * product1.setRights(rights2);
		 * 
		 * VideoInfo videoInfo = new VideoInfo(); videoInfo.setScid("0002663073"); videoInfo.setType("normal");
		 * videoInfo.setPixel("576x324"); videoInfo.setPictureSize("16:9"); videoInfo.setVersion("1");
		 * videoInfo.setBtvcid("2222222222"); videoInfo.setSize("307990233");
		 * 
		 * product1.setVod(new Vod(new Time("16", "63"), videoInfo)); subProjectList.add(product1);
		 */
		...
		for (VodDetail vodDetail : vodDetailList) {
			...
			// -------------------------------------------
			/*
			 * List<Source> sourceList = new ArrayList<Source>(); source = new Source();
			 * source.setMediaType(DisplayCommonUtil.getMimeType(vodDetail.getImgPath()));
			 * source.setSize(vodDetail.getImgSize()); source.setType(DisplayConstants.DP_SOURCE_TYPE_THUMBNAIL);
			 * source.setUrl(vodDetail.getImgPath()+vodDetail.getImgNm()); sourceList.add(source);
			 */
			// -------------------------------------------
			// screenshot
			// -------------------------------------------
			/*
			 * for (ProductImage screenshotImage : screenshotList) { Source screenshotSource = new Source();
			 * screenshotSource.setType(DisplayConstants.DP_SOURCE_TYPE_SCREENSHOT);
			 * screenshotSource.setSize(screenshotImage.getFileSize());
			 * screenshotSource.setUrl(screenshotImage.getFilePath() + screenshotImage.getFileNm());
			 * sourceList.add(screenshotSource); } subProduct.setSourceList(sourceList);
			 */
			...
			/** dwldAreaLimitYn 다운로드 지역제한 == 'Y' 일 경우 domestic 리턴 */
			if (StringUtils.isNotEmpty(vodDetail.getDwldAreaLimtYn()) && vodDetail.getDwldAreaLimtYn().equals("Y")) {
				rights.setAllow(DisplayConstants.DP_RIGHTS_ALLOW_DOMESTIC);
			}
