		...
		if (retMetaInfoList != null && retMetaInfoList.size() > 0) {
			...
			for (MetaInfo metaInfo : retMetaInfoList) {
				...
				TmembershipDcInfo info = this.displayCommonService.getTmembershipDcRateForMenu(header.getTenantHeader()
						.getTenantId(), metaInfo.getTopMenuId());
				List<Point> pointList = this.commonGenerator.generatePoint(info);
				// Tstore멤버십 적립율 정보
				// 정액제 패스/시리즈 패스만 조회
				if ((StringUtils.equals(DisplayConstants.FIXRATE_PROD_TYPE_VOD_FIXRATE, metaInfo.getCmpxProdClsfCd()) || StringUtils
						.equals(DisplayConstants.FIXRATE_PROD_TYPE_VOD_SERIESPASS, metaInfo.getCmpxProdClsfCd()))
