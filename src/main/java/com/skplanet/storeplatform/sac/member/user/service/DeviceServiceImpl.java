...
public class DeviceServiceImpl implements DeviceService {
	...
	public String updateDeviceInfo(SacRequestHeader requestHeader, DeviceInfo deviceInfo) {
		...
		if (StringUtils.isNotBlank(deviceInfo.getDeviceId()) && !StringUtils.equals(deviceInfo.getDeviceId(), userMbrDevice.getDeviceID())) {
		...
		if (StringUtils.equals(version, "v1")) {
			...
			if (StringUtils.isNotBlank(nativeId)) {
				...
				if (StringUtils.equals(MemberConstants.DEVICE_TELECOM_SKT, deviceTelecom)) {
					...
					} else if (StringUtils.equals(rooting, "Y") || StringUtils.equals(isNativeIdAuth, "Y")) { //isNativeIdAuth="Y"인경우 루팅여부관계없이비교
	 ...
	 */
	@Override
	public ChangedDeviceHistorySacRes searchChangedDeviceHistory(SacRequestHeader sacHeader, ChangedDeviceHistorySacReq request) {
		// 공통 파라미터 셋팅
		CommonRequest commonRequest = new CommonRequest();
		commonRequest.setSystemID(sacHeader.getTenantHeader().getSystemId());
		commonRequest.setTenantID(sacHeader.getTenantHeader().getTenantId());
