...
public class IdpServiceImpl implements IdpService {
	...
	public ImResult rXCreateUserIDP(HashMap<String, String> map) {
		...
		if ("null".equals(oldID) || "".equals(oldID)) { // 신규가입경우 기존 Tstore에 없던 회원가입요청시 전환가입 대상자중 Tstore 미가입자로 Tstore에
			...
			mbrOneID.setIntgMbrCaseCode(map.get("im_mem_type_cd").toString()); // 통합회원유형코드
	...
	public String changeMobileNumber(HashMap<String, String> map) {
		...
		try {
			...
			changeDeviceLog.setDeviceKey(schDeviceRes.getUserMbrDevice().getDeviceKey());
			...
			try {
				if (ex.getErrorInfo().getCode().equals(MemberConstants.SC_ERROR_NO_DATA)) {
					...
					try {
						...
						userMbrDevice.setChangeCaseCode(MemberConstants.DEVICE_CHANGE_TYPE_NUMBER_CHANGE); // 휴대기기 변경
																										   // 유형코드 :
																										   // 번호변경
	...
	public void insertIdpLog(CommonRequest commonRequest, ChangedDeviceLog changeDeviceLog) {
		...
		LOGGER.info(":::: createChangeDeviceReq : {}", createChangeDeviceReq.getCommonRequest().toString());
		LOGGER.info(":::: createChangeDeviceReq : {}", createChangeDeviceReq.getChangedDeviceLog().toString());
		...
		try {
			...
			try {
				...
				if (ex.getErrorInfo().getCode().equals(MemberConstants.SC_ERROR_NO_DATA)) {
					...
					LOGGER.info("<idpChangeMobile> NOT EXIST MEMBER. SVC_MGMT_NUM[ {} ]", svcMngNum);
	...
	public String joinComplete(HashMap<String, String> map) {
		...
		try {
			...
			changeDeviceLog.setSvcMangNum(svcMngNum);
			changeDeviceLog.setTenantID(tenantId);
			changeDeviceLog.setUserKey(schUserRes.getUserKey());
			// changeDeviceLog.setDeviceKey(deviceKey);
			// changeDeviceLog.setDeviceCode(deviceCode);
			// changeDeviceLog.setIsChanged(isChanged);
			this.insertIdpLog(commonRequest, changeDeviceLog);
		...
		try {
			...
			try {
				if (ex.getErrorInfo().getCode().equals(MemberConstants.SC_ERROR_NO_DATA)) {
					...
					changeDeviceLog.setSvcMangNum(svcMngNum);
					changeDeviceLog.setDeviceID(mdn);
					changeDeviceLog.setUserKey("NON_MEMBERS");
					// changeDeviceLog.setDeviceKey(deviceKey);
					// changeDeviceLog.setDeviceCode(deviceCode);
					// changeDeviceLog.setIsChanged(isChanged);
	...
	public ImResult rXUpdateAgreeUserIDP(HashMap<String, String> map) {
		LOGGER.debug("rXUpdateAgreeUserIDP ------- Start");
