...
public class LoginServiceImpl implements LoginService {
	...
	public AuthorizeByMdnRes authorizeByMdnV2(SacRequestHeader requestHeader, AuthorizeByMdnReq req) {
		...
		ChangedDeviceHistorySacRes changedDeviceHistoryRes = this.deviceService.srhChangedDeviceHistory(requestHeader,
		...
		if (chkDupRes.getUserMbr() == null && chkDupRes.getMbrOneID() == null) {
			...
			if (chkDupRes.getUserMbr() == null
					|| StringUtils.equals(chkDupRes.getUserMbr().getUserMainStatus(),
							MemberConstants.MAIN_STATUS_WATING)) { // 회원
																   // 정보가
																   // 없거나
																   // 회원메인상태가
																   // 가가입이면
				...
				try {
					...
					if (StringUtils.equals(authForIdEcRes.getCommonRes().getResult(), ImIdpConstants.IDP_RES_CODE_OK)) {
						...
						throw new StorePlatformException("SAC_MEM_1200"); // 원아이디 이용동의 간편가입 대상 정보가 상이합니다.
							...
							ImIdpConstants.IDP_RES_CODE_INVALID_USER_INFO)) { // 가가입
																			  // 상태인
																			  // 경우
																			  // EC에서
																			  // 성공으로
	...
	public AuthorizeSaveAndSyncByMacRes authorizeSaveAndSyncByMac(SacRequestHeader requestHeader,
		...
		if (StringUtils.equals(chkDupRes.getIsRegistered(), "Y")) { // 회원인 경우
			...
			DeviceInfo mdnDeviceInfo = this.deviceService.srhDevice(requestHeader, MemberConstants.KEY_TYPE_DEVICE_ID,
					req.getDeviceId(), null);
		...
		if (StringUtils.equals(isPurchaseChange, "Y") || StringUtils.equals(isVariability, "Y")) {
			...
			if (this.isCallChangeKey) {
				this.mcic.excuteInternalMethod(this.isCallChangeKey, requestHeader.getTenantHeader().getSystemId(),
						requestHeader.getTenantHeader().getTenantId(), joinForWapEcRes.getUserKey(), oldUserKey,
						oldDeviceKey, oldDeviceKey);
	 ...
	 */
	private String modDeviceInfoForLogin(SacRequestHeader requestHeader, String userKey, Object obj,
