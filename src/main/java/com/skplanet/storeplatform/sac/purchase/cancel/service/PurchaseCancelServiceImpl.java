...
public class PurchaseCancelServiceImpl implements PurchaseCancelService {
	...
	public PurchaseCancelSacResult cancelPurchaseList(PurchaseCancelSacParam purchaseCancelSacParam) {
		...
		for (PurchaseCancelDetailSacParam purchaseCancelDetailSacParam : purchaseCancelSacParam.getPrchsCancelList()) {
			...
			try {
				...
				/** 구매 취소 처리. */
				if (PurchaseConstants.PRCHS_CANCEL_SERVICE_TCASH.equals(purchaseCancelSacParam
						.getPrchsCancelServiceType())) {
			...
			if (StringUtils.equals("SAC_PUR_0000", purchaseCancelDetailSacResult.getResultCd())) {
				...
				if (StringUtils.startsWith(prchsDtlSacParam.getTenantProdGrpCd(),
					...
					this.purchaseDisplayRepository.updateSpecialPurchaseCount(prchsDtlSacParam.getTenantId(),
							...
							PurchaseConstants.PRCHS_STATUS_CANCEL, -purchaseCancelDetailSacParam
									.getPrchsDtlSacParamList().size(), prchsDtlSacParam.getPrchsDt(),
							purchaseCancelDetailSacParam.getCancelDt());
				}
			} else {
				failCnt++;
			}
	...
	public PurchaseCancelDetailSacResult executePurchaseCancel(PurchaseCancelSacParam purchaseCancelSacParam,
		...
		if (PurchaseConstants.PRCHS_CANCEL_BY_USER.equals(purchaseCancelSacParam.getPrchsCancelByType())) {
			// 사용자가 취소하는 경우 권한 체크.
	...
	private void setPaymentShopInfo(PurchaseCancelSacParam purchaseCancelSacParam,
		...
		for (PaymentSacParam paymentSacParam : purchaseCancelDetailSacParam.getPaymentSacParamList()) {
			...
			if (this.payPlanetShopService.startsWithPayPlanetMID(paymentSacParam.getTid())) {
				...
				PpProperty payPlanetShop = this.payPlanetShopService.getPayPlanetShopInfo(purchaseCancelDetailSacParam
						.getPrchsSacParam().getTenantId(), PurchaseConstants.PAYPLANET_API_TYPE_CANCEL,
