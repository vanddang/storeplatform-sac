...
public class AclServiceTobeImplTest {
	...
	@Mock
	private VerifyService requestValidateMock;
	...
	public void test() {
		...
		doThrow(new StorePlatformException("SAC_CMN_001")).when(this.requestValidateMock).verifyHeaders(headers);
		try {
			...
			verify(this.requestValidateMock).verifyHeaders(headers);
		}
	}
