...
public class ShoppingRepositoryImpl implements ShoppingRepository {
	...
	public CouponUseStatusSacResult getCouponUseStatus(CouponUseStatusSacParam couponUseStatusSacParam) {
		...
		this.logger.info("PRCHS,SHOPPING,SAC,CMS,GETCOUPONUSESTATUS,REQ,{}", couponUseStatusEcReq);
		CouponUseStatusEcRes couponUseStatusEcRes = this.shoppingSCI.getCouponUseStatus(couponUseStatusEcReq);
		this.logger.info("PRCHS,SHOPPING,SAC,CMS,GETCOUPONUSESTATUS,REQ,{}", couponUseStatusEcRes);
	...
	public CouponPublishAvailableSacResult getCouponPublishAvailable(
		...
		this.logger.info("PRCHS,SHOPPING,SAC,CMS,GETCOUPONPUBLISHAVAILABLE,REQ,{}", couponPublishAvailableEcReq);
		...
		this.logger.info("PRCHS,SHOPPING,SAC,CMS,GETCOUPONPUBLISHAVAILABLE,RES,{}", couponPublishAvailableEcRes);
	...
	public CouponPublishAvailableSacV2Result getCouponPublishAvailableV2(
		...
		this.logger.info("PRCHS,SHOPPING,SAC,CMS,GETCOUPONPUBLISHAVAILABLEV2,REQ,{}", couponPublishAvailableEcReq);
		...
		this.logger.info("PRCHS,SHOPPING,SAC,CMS,GETCOUPONPUBLISHAVAILABLEV2,RES,{}", couponPublishAvailableEcRes);
	...
	public CouponRestoreStatusSacResult restoreCouponStatus(CouponRestoreStatusSacParam couponRestoreStatusSacParam) {
		...
		try {
			this.logger.info("PRCHS,SHOPPING,SAC,CMS,RESTORECOUPONSTATUS,REQ,{}", couponRestoreStatusEcReq);
			couponUseStatusEcRes = this.shoppingSCI.restoreCouponStatus(couponRestoreStatusEcReq);
			this.logger.info("PRCHS,SHOPPING,SAC,CMS,RESTORECOUPONSTATUS,RES,{}", couponUseStatusEcRes);
			if(!StringUtils.equals(PurchaseConstants.TSTORE_COUPON_RESULT_CD_SUCCESS, couponUseStatusEcRes.getResultCd())){
				this.logger.info("PRCHS,SHOPPING,SAC,CMS,RESTORECOUPONSTATUS,ERROR,{},{}", couponRestoreStatusSacParam.getPrchsId(), couponUseStatusEcRes.getResultMsg());
				throw new StorePlatformException("SAC_PUR_1802");
			}
		} catch (Exception e){
				this.logger.info("PRCHS,SHOPPING,SAC,CMS,RESTORECOUPONSTATUS,ERROR,{}", couponRestoreStatusSacParam.getPrchsId());
		...
		for(String cpnPublicCd : StringUtils.splitPreserveAllTokens(StringUtils.defaultString(couponUseStatusEcRes.getCpnPublishCd()), PurchaseConstants.SEPARATOR))
		{
			...
			try {
				...
				if(count != 1) {
					this.logger.info("PRCHS,SHOPPING,SAC,DB,RESTORECOUPONSTATUS,ERROR,{},{}", couponRestoreStatusSacParam.getPrchsId(), cpnPublicCd);
