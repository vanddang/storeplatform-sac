...
public class AppCodiServiceImpl implements AppCodiService {
	...
	private Product getMetaInfo( ProductBasicInfo info, SacRequestHeader header ) {
		...
		if (DisplayConstants.DP_GAME_TOP_MENU_ID.equals(topMenuId)
			...
			metaInfo = this.metaInfoService.getAppMetaInfo(info);
		...
		// 영화
			...
			metaInfo = this.metaInfoService.getVODMetaInfo(info);
		...
		// 이북
			...
			metaInfo = this.metaInfoService.getEbookComicMetaInfo(info);
		...
		// 음악
			...
			metaInfo = this.metaInfoService.getMusicMetaInfo(info);
