...
public class LoginServiceImpl implements LoginService {
	...
	public AuthorizeByMdnRes authorizeByMdn(SacRequestHeader requestHeader, AuthorizeByMdnReq req) {
        ...
        searchExtentUserRequest.setKeySearchList(userSearchList);
		...
		/** 06. 로그인 성공이력 저장 */
		this.regLoginHistory(requestHeader, searchExtentUserResponse.getUserMbr().getUserKey(), null, "Y", "Y", req.getDeviceIp(),
		...
		/** 9. 로그인 결과 */
		res.setUserKey(searchExtentUserResponse.getUserMbr().getUserKey());
		res.setUserType(searchExtentUserResponse.getUserMbr().getUserType());
		res.setUserMainStatus(searchExtentUserResponse.getUserMbr().getUserMainStatus());
		res.setUserSubStatus(searchExtentUserResponse.getUserMbr().getUserSubStatus());
	...
	public AuthorizeByMdnRes authorizeByMdnV2(SacRequestHeader requestHeader, AuthorizeByMdnReq req) {
        ...
        if (StringUtils.equals(req.getDeviceTelecom(), MemberConstants.DEVICE_TELECOM_SKT)
                && !StringUtils.equals(this.isAgreementByAgreementCode(requestHeader, searchExtentUserResponse.getUserMbr()
                .getUserKey(), MemberConstants.POLICY_AGREEMENT_CLAUSE_INDIVIDUAL_INFO_HANDLE_OTHERS, searchExtentUserResponse
        ...
        if(StringUtils.equals(req.getDeviceTelecom(), MemberConstants.DEVICE_TELECOM_SKM)
            ...
            if(deviceInfo != null){
				...
				if(isTempLoginSucc){
                    ...
                    /* 로그인 성공이력 저장 */
                    this.regLoginHistory(requestHeader, deviceInfo.getUserKey(), null, "Y", "Y", req.getDeviceIp(), null, null, "Y", deviceInfo.getDeviceKey(), req.getDeviceType());
	...
	public AuthorizeByIdRes authorizeById(SacRequestHeader requestHeader, AuthorizeByIdReq req) {
		...
		chkDupReq.setCommonRequest(this.commService.getSCCommonRequest(requestHeader));
		chkDupReq.setKeySearchList(keySearchList);
		chkDupReq.setUserType(MemberConstants.USER_TYPE_TSTORE); // ID 기반 회원 인증은 OneStore회원만 해당된다.
		...
		if (StringUtils.equals(chkUserPwdRes.getUserKey(), userKey)) {
			...
			this.regLoginHistory(requestHeader, userKey, userPw, "Y", "N", req.getIpAddress(), "N", null, "Y", null, null);
		...
		/** 4-2. pwd 불일치 - 로그인 실패 */
			...
			this.regLoginHistory(requestHeader, userKey, userPw, "N", "N", req.getIpAddress(), "N", null, "N", null, null);
	...
	public AuthorizeByIdV2SacRes authorizeByIdV2(SacRequestHeader requestHeader, AuthorizeByIdV2SacReq req) {
		...
		if(StringUtils.isNotBlank(req.getUserAuthToken())){ // userAuthToken이 넘어온 경우만 유효성 체크
			if (StringUtils.equals(req.getUserType(), MemberConstants.USER_TYPE_TSTORE)){
				...
				chkUserAuthTkReqeust.setUserKey(searchExtentUserResponse.getUserMbr().getUserKey());
				chkUserAuthTkReqeust.setUserAuthToken(req.getUserAuthToken());
				chkUserAuthTkReqeust.setIsDormant(searchExtentUserResponse.getUserMbr().getIsDormant());
				try {
				...
				List<UserExtraInfo> userExtraInfoList = userExtraInfoService.findExtraInfo(searchExtentUserResponse.getUserMbr().getUserKey());
        ...
        schDeviceListReq.setUserKey(searchExtentUserResponse.getUserMbr().getUserKey());
        ...
        key.setKeyString(searchExtentUserResponse.getUserMbr().getUserKey());
            ...
            if(searchDeviceListResponse != null && searchDeviceListResponse.getUserMbrDevice().size() > 0){
                for (UserMbrDevice userMbrDevice : searchDeviceListResponse.getUserMbrDevice()) {
                    if(StringUtils.equals(userMbrDevice.getIsUsed(), MemberConstants.USE_Y) && StringUtils.equals(req.getMdn(), userMbrDevice.getMdn())){
						...
						boolean isTempLoginSucc = false;
						boolean isValidTelecom = false;
						if(StringUtils.equals(userMbrDevice.getDeviceID(), req.getDeviceId())){
							isTempLoginSucc = true;
						}else{
							if(StringUtils.equals(req.getDeviceTelecom(), MemberConstants.DEVICE_TELECOM_SKM)){
						...
						if(isTempLoginSucc){
							// 이메일 정보 업데이트
							if(StringUtils.isNotBlank(req.getUserEmail()) && !StringUtils.equals(req.getUserEmail(), searchExtentUserResponse.getUserMbr().getUserEmail())){
								UserMbr userMbr = new UserMbr();
								userMbr.setUserKey(searchExtentUserResponse.getUserMbr().getUserKey());
								...
								LOGGER.info("이메일 정보 변경 {} -> {}", searchExtentUserResponse.getUserMbr().getUserEmail(), req.getUserEmail());
							}
							...
							// 로그인 성공이력 저장
							this.regLoginHistory(requestHeader, searchExtentUserResponse.getUserKey(), null, "Y", "Y", req.getDeviceIp(), null, null, "Y", userMbrDevice.getDeviceKey(), req.getDeviceType());
        ...
        if(StringUtils.equals(MemberConstants.DEVICE_TELECOM_SKT, req.getDeviceTelecom())
            ...
            if(deviceTelecomInfo == null){
                ...
                if(searchDeviceListResponse != null && searchDeviceListResponse.getUserMbrDevice().size() > 0) {
                    ...
                    if(isTempLoginSucc){
                        // 로그인 이력 저장
                        this.regLoginHistory(requestHeader, searchExtentUserResponse.getUserKey(), null, "Y", "N", req.getDeviceIp(), "N", null, "Y", deviceKey, req.getDeviceType());
		...
		deviceInfo.setUserKey(searchExtentUserResponse.getUserMbr().getUserKey());
	...
	public AuthorizeForInAppSacRes authorizeForInApp(SacRequestHeader requestHeader,
		...
		if (StringUtils.equals(res.getUserStatus(), MemberConstants.INAPP_USER_STATUS_NORMAL)) {
			this.regLoginHistory(requestHeader, res.getUserInfo().getUserKey(), null, "Y", "Y", req.getDeviceId(), "N", "", "N", null, null);
		}
	...
	public AuthorizeForInAppSacRes authorizeForInAppV2(SacRequestHeader requestHeader,
		...
		// 로그인 이력 저장
		if (StringUtils.equals(res.getUserStatus(), MemberConstants.INAPP_USER_STATUS_NORMAL)) {
			this.regLoginHistory(requestHeader, res.getUserInfo().getUserKey(), null, "Y", "Y", res.getDeviceId(), "N", "", "N", null, null);
		}
	...
	public AuthorizeForInAppSacRes authorizeForInAppV3(SacRequestHeader requestHeader,
		...
		// 로그인 이력 저장
		if (StringUtils.equals(res.getUserStatus(), MemberConstants.INAPP_USER_STATUS_NORMAL)) {
			this.regLoginHistory(requestHeader, res.getUserInfo().getUserKey(), null, "Y", "Y", res.getDeviceId(), "N", "", updLastLoginDtYn, res.getDeviceInfo().getDeviceKey(), null);
		}
		return res;
	}
	...
	public AuthorizeForOllehMarketSacRes authorizeForOllehMarket(SacRequestHeader requestHeader,
		...
		if (marketRes != null) {
			...
			if (StringUtils.equals(marketRes.getUserStatus(), MemberConstants.INAPP_USER_STATUS_NORMAL)) {
				...
				// 로그인 이력 저장
				this.regLoginHistory(requestHeader, res.getUserInfo().getUserKey(), null, "Y", "Y", marketRes.getDeviceId(),
	...
	public AuthorizeV2SacRes authorizeV2(SacRequestHeader requestHeader, @Valid @RequestBody AuthorizeV2SacReq req) {
		...
		if (StringUtils.equals(res.getUserMainStatus(), MemberConstants.INAPP_USER_STATUS_NORMAL)) {
			this.regLoginHistory(requestHeader, res.getUserInfo().getUserKey(), null, "Y", "Y", res.getDeviceInfo()
	...
	private AuthorizeForInAppSacRes getTstoreMemberInfoForInApp(SacRequestHeader requestHeader, AuthorizeForInAppSacReq req) {
		...
		// PIN 정보
        MarketPinInfo pinInfo = this.getMarketPinInfo(detailRes.getUserKey(), detailRes.getDeviceInfoList().get(0).getDeviceKey());
	 ...
	 */
	private LoginUserResponse regLoginHistory(SacRequestHeader requestHeader, String userKey, String userPw,
	...
	public AuthorizeByPwdSacRes authorizeByPassword(SacRequestHeader requestHeader, AuthorizeByPwdSacReq req){
		...
		if (StringUtils.equals(chkUserPwdRes.getUserKey(), userKey)) {
			...
			this.regLoginHistory(requestHeader, userKey, userPw, "Y", "N", null, "N", null, "Y", null, null);
		...
		/** 3-2. pwd불일치 - 로그인 실패 */
			...
			this.regLoginHistory(requestHeader, userKey, userPw, "N", "N", null, "N", null, "N", null, null);
