...
//		}
	...
	public CheckSaveNSyncResponse checkSaveNSync(CheckSaveNSyncRequest checkSaveNSyncRequest) {
		...
		// 테넌트 아이디 없음
		if (checkSaveNSyncRequest.getCommonRequest().getTenantID() == null
				|| checkSaveNSyncRequest.getCommonRequest().getTenantID().length() <= 0) {
			throw new StorePlatformException(this.getMessage("response.ResultCode.tanentIDNotFound", ""));
		}
	...
	public SearchDeviceOwnerResponse searchDeviceOwner(SearchDeviceOwnerRequest searchDeviceOwnerRequest) {
		...
		// 테넌트 아이디 없음
		if (searchDeviceOwnerRequest.getCommonRequest().getTenantID() == null
				|| searchDeviceOwnerRequest.getCommonRequest().getTenantID().length() <= 0) {
			throw new StorePlatformException(this.getMessage("response.ResultCode.tanentIDNotFound", ""));
		}
	...
	public SearchOrderDeviceResponse searchOrderDevice(SearchOrderDeviceRequest searchOrderDeviceRequest) {
		...
		// 테넌트 아이디 없음
		if (searchOrderDeviceRequest.getCommonRequest().getTenantID() == null
				|| searchOrderDeviceRequest.getCommonRequest().getTenantID().length() <= 0) {
			throw new StorePlatformException(this.getMessage("response.ResultCode.tanentIDNotFound", ""));
		}
	...
	public UpdateDeviceManagementResponse updateDeviceManagement(
		...
		// 테넌트 아이디 없음
		if (updateDeviceManagementRequest.getCommonRequest().getTenantID() == null
				|| updateDeviceManagementRequest.getCommonRequest().getTenantID().length() <= 0) {
			throw new StorePlatformException(this.getMessage("response.ResultCode.tanentIDNotFound", ""));
		}
    ...
    public SearchDeviceMvnoResponse searchDeviceMvno(SearchDeviceMvnoRequest searchDeviceMvnoRequest){
        LOGGER.debug("\n\n\n\n\n");
        LOGGER.debug("==================================================================================");
        LOGGER.debug("디바이스 컨트롤러 - MVNO 휴대기기 개별 조회");
        LOGGER.debug("==================================================================================\n\n\n\n\n");

        LOGGER.debug("### client에서 받은값 : {}", searchDeviceMvnoRequest);

        SearchDeviceMvnoResponse searchDeviceMvnoResponse = null;

        // 입력 파라미터가 없음
        if (searchDeviceMvnoRequest == null) {
            throw new StorePlatformException(this.getMessage("response.ResultCode.inputNotFound", ""));
        }

        // 공통 파라미터 없음
        if (searchDeviceMvnoRequest.getCommonRequest() == null) {
            throw new StorePlatformException(this.getMessage("response.ResultCode.commonNotFound", ""));
        }

        // mdn 체크
        if(searchDeviceMvnoRequest.getMdn() == null || searchDeviceMvnoRequest.getMdn().length() <= 0){
            throw new StorePlatformException(this.getMessage("response.ResultCode.commonNotFound", ""));
        }

        // mdn 체크
        if(searchDeviceMvnoRequest.getDeviceNatvId() == null || searchDeviceMvnoRequest.getDeviceNatvId().length() <= 0){
            throw new StorePlatformException(this.getMessage("response.ResultCode.commonNotFound", ""));
        }
        try {

            searchDeviceMvnoResponse = this.device.searchDeviceMvno(searchDeviceMvnoRequest);
